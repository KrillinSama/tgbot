...
from axel_kassemsyr.modules.helper_funcs.msg_types import Types
from axel_kassemsyr.modules.sql import SESSION, BASE

DEFAULT_WELCOME = "Hey there {first}, and welcome to {chatname}! How are you?"
DEFAULT_GOODBYE = "GTFO Bitchhhhh!"
...
class WelcomeMute(BASE):
    ...
    chat_id = Column(String(14), primary_key=True)
    welcomemutes = Column(Integer, default=False)
...
def check_user(user_id):
    status = SESSION.query(VerifiedUser).get(str(user_id))
    SESSION.close()
    if status:
        return True
    return False
