...
from axel_kassemsyr import dispatcher, BAN_STICKER, LOGGER
from axel_kassemsyr.modules.disable import DisableAbleCommandHandler
from axel_kassemsyr.modules.helper_funcs.chat_status import bot_admin, user_admin, is_user_ban_protected, can_restrict, \
    is_user_admin, is_user_in_chat, is_bot_admin
from axel_kassemsyr.modules.helper_funcs.extraction import extract_user_and_text
from axel_kassemsyr.modules.helper_funcs.string_handling import extract_time
...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        ...
        keyboard = []
        bot.send_sticker(update.effective_chat.id, BAN_STICKER)  #axel ban sticker
        reply = "I've banned {} ".format(mention_html(member.user.id, member.user.first_name))
...
def temp_ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    try:
        ...
        bot.send_sticker(chat.id, BAN_STICKER)  # axel ban sticker
...
def kick(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if res:
        bot.send_sticker(chat.id, BAN_STICKER)  # axel ban sticker
        keyboard = []
        reply = "I've kicked {} ".format(mention_html(member.user.id, member.user.first_name))
        message.reply_text(reply, reply_markup=keyboard, parse_mode=ParseMode.HTML)
