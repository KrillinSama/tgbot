...
import axel_kassemsyr.modules.sql.notes_sql as sql
from axel_kassemsyr import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, MESSAGE_DUMP
from axel_kassemsyr.__main__ import DATA_IMPORT
from axel_kassemsyr.modules.helper_funcs.chat_status import user_admin
from axel_kassemsyr.modules.helper_funcs.misc import build_keyboard, revert_buttons
from axel_kassemsyr.modules.helper_funcs.msg_types import get_note_type
from axel_kassemsyr.modules.rules import get_rules
import axel_kassemsyr.modules.sql.rules_sql as rulessql
from axel_kassemsyr.modules.sql import warns_sql as warnssql
import axel_kassemsyr.modules.sql.blacklist_sql as blacklistsql
from axel_kassemsyr.modules.sql import disable_sql as disabledsql
from axel_kassemsyr.modules.sql import cust_filters_sql as filtersql
import axel_kassemsyr.modules.sql.welcome_sql as welcsql
import axel_kassemsyr.modules.sql.locks_sql as locksql
...
def import_data(bot: Bot, update):
	...
	if msg.reply_to_message and msg.reply_to_message.document:
		...
		try:
			...
			return msg.reply_text("There is problem while importing the data! Please ask in @axel_kassemsyrAyaGroup about why this happened.")
		# Check if backup is from self
		try:
			if str(bot.id) != str(data[str(chat.id)]['bot']):
				return msg.reply_text("Backup from another bot that is not suggested might cause the problem, documents, photos, videos, audios, records might not work as it should be. However, You can still request a feature regarding this in @axel_kassemsyrAyaGroup !")
		except:
			pass
		# Select data source
		...
		try:
			...
			msg.reply_text("An error occurred while recovering your data. The process failed. If you experience a problem with this, please ask in @axel_kassemsyrAyaGroup . My owner and community will be happy to help. Also, bugs report makes me even better!\nThank you!")
...
def export_data(bot: Bot, update: Update, chat_data):
	...
	f=open("axel_kassemsyrb{}.backup".format(chat_id), "w")
	...
	try:
		...
		pass
	bot.sendDocument(current_chat_id, document=open('axel_kassemsyrb{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `axel_kassemsyrb` is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
	os.remove("axel_kassemsyrb{}.backup".format(chat_id)) # Cleaning file
