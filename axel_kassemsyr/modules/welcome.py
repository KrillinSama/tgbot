...
import axel_kassemsyr.modules.helper_funcs.cas_api as cas
...
import axel_kassemsyr.modules.sql.welcome_sql as sql
import axel_kassemsyr.modules.sql.global_bans_sql as gbansql
import axel_kassemsyr.modules.sql.users_sql as userssql

from axel_kassemsyr import dispatcher, OWNER_ID, LOGGER, SUDO_USERS, SUPPORT_USERS
from axel_kassemsyr.modules.helper_funcs.chat_status import user_admin, can_delete, is_user_ban_protected
from axel_kassemsyr.modules.helper_funcs.misc import build_keyboard, revert_buttons, send_to_list
from axel_kassemsyr.modules.helper_funcs.msg_types import get_welcome_type
...
from axel_kassemsyr.modules.helper_funcs.string_handling import markdown_parser, escape_invalid_curly_brackets
from axel_kassemsyr.modules.log_channel import loggable
    ...
    if casPrefs and not autoban and cas.banchecker(user.id):
        ...
        for new_mem in new_members:
            ...
            # Make bot greet admins
                ...
                status = sql.check_user(new_mem.id)	
                if is_user_ban_protected(chat, new_mem.id, chat.get_member(new_mem.id)):
                	continue
                if status:
                	continue
                if welc_mutes == "on":
                    msg.reply_text("You will be restricted until you prove that you'r a human",
                         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Click here to prove you'r human", 
...
@user_admin
@loggable
def welcome_security(bot: Bot, update: Update, args: List[str]) -> str:
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
    msg = update.effective_message # type: Optional[Message]
...
def user_button(bot: Bot, update: Update):
    ...
    if join_user == user.id:
        query.answer(text="Welcome to the chat!")
        ...
        query.answer(text="Oh Really?! This buttom isn't for you!")
...
*Admin only:*
 ...
 - /welcomemute <on/off>: all users that join for the first time, get muted; a button gets added to the welcome message for them to unmute themselves, if not they will be kicked.
...
WELCOME_MUTE_HANDLER = CommandHandler("welcomemute", welcome_security, pass_args=True, filters=Filters.group)
